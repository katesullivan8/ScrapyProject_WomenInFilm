import scrapy
from bechdel.items import ImdbItem
...
class ImdbSpider(Spider):
  ...
  def parse(self, response):
    ...
    decades_list = ['https://www.imdb.com/search/title?title_type=feature&release_date={}-01-01,{}-12-31&view=simple&sort=moviemeter'.format(x, x+9) for x in range(2009, 1998, -10)]
  ...
  def cast_crew_detail(self, response):
    ...
    table_rows = response.xpath('//h4[@class="dataHeaderWithBorder"]/text()').extract()
    ...
    if dept_tags[0] in table_rows:
      dir_number = table_rows.index(dept_tags[0])
    if dept_tags[1] in table_rows:
      prod_number = table_rows.index(dept_tags[1])
    if dept_tags[2] in table_rows:
      cam_number = table_rows.index(dept_tags[2])
    if dept_tags[3] in table_rows:
      prod_manage_num = table_rows.index(dept_tags[3])
    prod_names = []
    dir_names = []
    camera_team = []
    prod_manage_team = []
